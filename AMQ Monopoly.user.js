// ==UserScript==
// @name         AMQ Monopoly Soft Vers today
...
let commandListener = new Listener("Game Chat Message", (payload) => {
    if (payload.sender === selfName) {
        ...
        else if (payload.message.startsWith("/setwin")) {
            let message = payload.message.split(" ");
...
function tileEventHandler(current){
    ...
    if (current == 0 || tile.description.startsWith("Just Visiting")){
        ...
        modifyMessage = "Anime settings Randomized";
        randomize();
    }
...
function hasRandomize(condition){
    return condition.modifier === "Anime settings Randomized";
}
