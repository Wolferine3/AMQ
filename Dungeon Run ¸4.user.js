...
function answerResultsListenerAction(result) {
    ...
    if (Math.max(...scores) === 0) {
        ...
        inDungeon = false;
        inParty = false;
        xpListener.unbindListener();
    }
    else if (Math.max(...scores) === 1 || Math.max(...scores) === 2 || Math.max(...scores) === 3 || Math.max(...scores) === 4) {
        sendChatMessage("The monster is faster than you thought and a careless mistake led to "+newMonster[0]+" unleashing an AOT attack.");
