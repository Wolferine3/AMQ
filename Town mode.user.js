...
let commandListener = new Listener("Game Chat Message", (payload) => {
    ...
    if (payload.sender === selfName && payload.message.startsWith("/roles")) {
        ...
        for (let i = 0; i < information.length; i++) {
            ...
            if (indexOfMafias > -1) {
                positionOfMafias.push([i]);
            }
        }
        setTimeout(function(){ sendDMMessage("The other mafia is: "+information[positionOfMafias[1]][0], information[positionOfMafias[0]][0]); }, 8000);
        setTimeout(function(){ sendDMMessage("The other mafia is: "+information[positionOfMafias[0]][0], information[positionOfMafias[1]][0]); }, 9000);
        //Finding the index of Investigator
        for (let i = 0; i < information.length; i++) {
            var indexOfInvestigator = information[i].indexOf("Investigator");
            if (indexOfInvestigator > -1) {
                positionOfInvestigator.push([i]);
            }
        }
        ...
        for (let i = 0; i < information.length; i++) {
            ...
            if (indexOfCovenInvestigator > -1) {
                positionOfCovenInvestigator.push([i]);
            }
        }
        setTimeout(function(){ sendDMMessage("The Coven Leader is: "+information[positionOfCovenLeader[0]][0], information[positionOfCovenInvestigator[0]][0]); }, 10000);
        //Finding the index of Coven Leader
        for (let i = 0; i < information.length; i++) {
            var indexOfCovenLeader = information[i].indexOf("Coven Leader");
            if (indexOfCovenLeader > -1) {
                positionOfCovenLeader.push([i]);
            }
        }
        setTimeout(function(){ sendDMMessage("The Coven Investigator is: "+information[positionOfCovenInvestigator[0]][0], information[positionOfCovenLeader[0]][0]); }, 11000);
...
//let songNumber = $("#qpCurrentSongCount").text()+1;
...
let dmListener = new Listener("chat message", function (payload) {
    ...
    if (payload.message.startsWith("/stun")) {
        ...
        if (payload.sender === information[positionOfJailors[0]][0]) {
            //if (powerJailor1Counter = false) {
            sendDMMessage("You are stunned on song "+(parseInt($("#qpCurrentSongCount").text())+1).toString()+".", target);
        ...
        else if (payload.sender === information[positionOfJailors[1]][0]) {
        ...
        else if (payload.sender === information[positionOfCovenLeader[0]][0]) {
    ...
    if (payload.message.startsWith("/inv")) {
        if (payload.sender === information[positionOfInvestigator[0]][0]) {
            ...
            if ((information[positionOfTarget[0]][1] === "Sheriff") || (information[positionOfTarget[0]][1] === "Investigator") || (information[positionOfTarget[0]][1] === "Jailor")) {
    ...
    if (payload.message.startsWith("/sinv")) {
        if (payload.sender === information[positionOfSheriff[0]][0]) {
    ...
    if (payload.message.startsWith("/cinv")) {
        if (payload.sender === information[positionOfCovenInvestigator[0]][0]) {
            ...
            sendDMMessage("Target's role is "+information[positionOfTarget[0]][1], payload.sender);
        }
        //else {
        // sendDMMessage("You are not allowed to use your power right now.", payload.sender);
        // }
...
let resultListener = new Listener("answer results", function (result) {
    ...
    //To access a player score live during the game, we need a delay.

    setTimeout(answerResultsListenerAction(result), 6000);
...
function answerResultsListenerAction(result) {
    ...
    if ((information[positionOfJailors[0]][2] == 0) && (information[positionOfJailors[1]][2] == 0) && (deathTownCounter = false)) {
        swap(roles, information[positionOfJailors[0]][1], information[positionOfInvestigator[0]][1]);
        swap(roles, information[positionOfJailors[1]][1], information[positionOfSheriff[0]][1]);
        sendDMMessage("You are now Jailor.", information[positionOfJailors[0]][0]);
        setTimeout(function(){ sendDMMessage("You are now Jailor.", information[positionOfJailors[1]][0]); }, 1000);
        deathTownCounter = true;
    }
    //Same for Coven Leader / Coven Investigator
    let deathCovenCounter = false;
    if ((information[positionOfCovenLeader[0]][2] == 0) && (deathCovenCounter = false)) {
        swap(roles, information[positionOfCovenLeader[0]][1], information[positionOfCovenInvestigator[0]][1]);
        sendDMMessage("You are now Coven Leader.", information[positionOfCovenLeader[0]][0]);
        deathCovenCounter = true;
    }
    //Send Death Messages
    let deathSheriffCounter = false;
    if ((information[positionOfSheriff[0]][2] == 0) && (deathSheriffCounter = false)) {
        //sendChatMessage("Player "+information[positionOfSheriff[0]][0]+" has died. They were the Sheriff.");
        deathSheriffCounter = true;
    }
    let deathInvestigatorCounter = false;
    if ((information[positionOfInvestigator[0]][2] == 0) && (deathInvestigatorCounter = false)) {
        //sendChatMessage("Player "+information[positionOfInvestigator[0]][0]+" has died. They were the Investigator.");
        deathInvestigatorCounter = true;
    }
    let deathCovenInvestigatorCounter = false;
    if ((information[positionOfCovenInvestigator[0]][2] == 0) && (deathCovenInvestigatorCounter = false)) {
        //sendChatMessage("Player "+information[positionOfCovenInvestigator[0]][0]+" has died. They were the Coven Investigator.");
        deathCovenInvestigatorCounter = true;
    }
    let deathMafia1Counter = false;
    if ((information[positionOfMafias[0]][2] == 0) && (deathMafia1Counter = false)) {
        //sendChatMessage("Player "+information[positionOfMafias[0]][0]+" has died. They were a Mafia member.");
        deathMafia1Counter = true;
    }
    let deathMafia2Counter = false;
    if ((information[positionOfMafias[1]][2] == 0) && (deathMafia2Counter = false)) {
        //sendChatMessage("Player "+information[positionOfMafias[1]][0]+" has died. They were a Mafia member.");
        deathMafia2Counter = true;
    }
    //Since Jailor role gets swapped
    let deathJailor1Counter = 0;
    if ((information[positionOfJailors[0]][2] == 0) && ((deathJailor1Counter = 0) || (deathJailor1Counter = 1))) {
        //sendChatMessage("Player "+information[positionOfJailors[0]][0]+" has died. They were a Jailor.");
        deathJailor1Counter++;
    }
    let deathJailor2Counter = 0;
    if ((information[positionOfJailors[1]][2] == 0) && ((deathJailor2Counter = 0) || (deathJailor2Counter = 1))) {
        //sendChatMessage("Player "+information[positionOfJailors[1]][0]+" has died. They were a Jailor.");
        deathJailor2Counter++;
    }
    //Same for Coven Leader
    let deathCovenLeaderCounter = 0;
    if ((information[positionOfCovenLeader[0]][2] == 0)&& ((deathCovenLeaderCounter = 0) || (deathCovenLeaderCounter = 1))) {
        //sendChatMessage("Player "+information[positionOfCovenLeader[0]][0]+" has died. They were the Coven Leader.");
