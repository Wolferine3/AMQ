...
let commandListener = new Listener("Game Chat Message", (payload) => {
    if (payload.message.startsWith("/start") && payload.sender === selfName) {
        ...
        let commandListener2 = new Listener("Game Chat Message", (payload) => {
            if (payload.message.startsWith("/") && payload.sender === selfName) {
                ...
                firstMonster = monsters[positionOfMonster[0]];
                sendChatMessage("A wild "+firstMonster[0]+" appears.");

                levelOfMonster = getRandomIntInclusive(firstMonster[1],firstMonster[2]);
                baseHP = Math.round(100/3*levelOfMonster);
                HP = baseHP;
                sendChatMessage(firstMonster[0]+" Lv."+levelOfMonster);
                sendChatMessage("🟩🟩🟩🟩🟩🟩🟩🟩"+HP/baseHP*100+"%");
                setDifficulty([firstMonster[4],firstMonster[5]]);
                ...
                let _endResultListener = new Listener("quiz end result", function (payload) {
                    while (lobby.inLobby == false) {
                        setTimeout(function(){wait(), 2000});
                    }
                    for (let i = 0; i < dungeonRunLength-1; i++) {
                        selectNewMonster();
                        newMonster = monsters[positionOfNewMonster[0]];
...
function wait() {
    //I'm dumb and lazy so idk how to make the system wait properly
}

function selectNewMonster() {
    let positionOfNewMonster = positionOfMonster[0] + getRandomIntInclusive(1,3);
    return positionOfNewMonster;
}
