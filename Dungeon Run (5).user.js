...
let monsterList = ["Spider", "Bee", "Rat", "Bat", "Slime", "Snake", "Wolf", "Goblin", "Skeleton", "Ghoul", "Mummy", "Ghost", "Hobgoblin", "Kobold", "Siren", "Wraith", "Troll", "Orc", "Harpy",
                   "Gargoyle", "Minotaur", "Werewolf", "Cockatrice", "Golem", "Wyvern", "Ogre", "Vampire", "Griffin", "Lich", "Demon", "Hydra", "Phoenix", "Dragon", "Pingu"];
let levelMin = [1, 1, 2, 2, 3, 4, 5, 5, 6, 6, 6, 7, 9, 9, 10, 12, 15, 16, 17, 17, 20, 22, 24, 30, 35, 40, 42, 48, 55, 65, 70, 75, 80, 85];
let levelMax = [2, 3, 4, 5, 5, 6, 7, 8, 8, 9, 10, 10, 12, 12, 13, 16, 18, 20, 20, 21, 25, 26, 30, 35, 40, 45, 50, 58, 60, 70, 75, 80, 90, 100];
...
let commandListener = new Listener("game chat update", (payload) => {
    if (payload.messages[0].message.startsWith("/start") && payload.messages[0].sender === selfName && inDungeon == false) {
        ...
        let commandListener2 = new Listener("game chat update", (payload) => {
            if (payload.messages[0].message.startsWith("/") && payload.messages[0].sender === selfName && inParty == false) {
                inParty = true;
                inBattle = true;
                ...
                sendChatMessage("---------------------------------------------------");
                if (numberOfPlayers == 8 || numberOfPlayers == 7 || numberOfPlayers == 6) {
...
function rightAnswerListenerAction(data) {
    normalAttack = getRandomGIntInclusive(10, baseAttackPower - 0.2*baseAttackPower, baseAttackPower + 0.2*baseAttackPower);
    critAttack = normalAttack*3;
    if (getRandomIntInclusive(0,9) == 0){
        attackPower = critAttack;
        sendChatMessage("---------------------------------------------------");
        sendChatMessage("Critical hit! You dealt "+attackPower+" damage!");
    }
    else {
        attackPower = normalAttack;
        sendChatMessage("---------------------------------------------------");
...
let moodAdjectives = ["An accomplished", "An amused", "An angry", "An annoyed", "An anxious", "An apathetic", "An ashamed", "A bewildered", "A bitchy", "A bittwersweet", "A blissful", "A bored",
                      "A bouncy", "A calm", "A cheerful", "A confused", "A cranky", "A crazy", "A curious", "A cynical", "A depressed", "A determined", "A devious", "A disappointed", "A drunk",
                      "An ectatic", "An enraged", "An envious", "An excited", "A flirty", "A frustrated", "A giggly", "A gloomy", "A grateful", "A grumpy", "A guilty", "A happy", "A hopeful",
                      "A hungry", "An impressed", "An indifferent", "An infuriated", "An irritated", "A jealous", "A lazy", "A lethargic", "A lonely", "A loved", "A mad", "A melancholic", "A mellow",
                      "A mischievous", "A naughty", "An optimistic", "A peaceful", "A pessimistic", "A pissed off", "A rejected", "A relaxed", "A relieved", "A restless", "A sad", "A satisfied",
                      "A shocked", "A sleepy", "A surprised", "A thankful", "A tired", "An uncomfortable", "A weird"];
function partyAdvances(battleNumber) {
    ...
    score3 = false;
    score4 = false;
    let positionOfAdjective = getRandomIntInclusive(0,moodAdjectives.length);
    sendChatMessage(moodAdjectives[positionOfAdjective]+" "+newMonster[0]+" appears.");
